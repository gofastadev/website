...
5. [The Framework Library](#5-the-framework-library)
...
- **Not a lock-in.** Every generated file is owned by the developer. Any framework package can be replaced with a third-party alternative or a custom implementation. The CLI generates code once; it does not manage or update generated code after the fact.
...
| `pkg/notifications` | Multi-channel notifications (email, database, Slack, push) |
| `pkg/websocket` | WebSocket hub with rooms, broadcast, and direct messaging |
| `pkg/sessions` | Cookie and Redis session management |
| `pkg/scheduler` | Cron-based job scheduling with concurrency control |
| `pkg/queue` | Async task queues with Redis or in-memory backends |
| `pkg/resilience` | Circuit breaker, retry with backoff, bulkhead, timeout |
| `pkg/validators` | Chainable field validation with built-in rules |
| `pkg/i18n` | Internationalization with JSON translation files and plural forms |
| `pkg/observability` | Prometheus metrics and OpenTelemetry distributed tracing |
| `pkg/feature-flags` | File-based or HTTP-based feature flags with percentage rollouts |
...
| `pkg/test-utilities` | Test database setup, HTTP test client, fixtures, assertions |
...
All models embed `models.BaseModelImpl` from the framework library, which provides:
...
- `gofasta upgrade` command to update framework packages in existing projects
...
**Repositories:**
- **Framework library:** `github.com/gofastadev/gofasta`
- **CLI tool:** `github.com/gofastadev/cli`
