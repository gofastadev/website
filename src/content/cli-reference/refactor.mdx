...
1. **Preconditions** — `project.layout == "layered"` (or empty + `app/models/` exists); git tree clean (or `--force`); the [eligibility preflight](#eligibility-preflight) passes — blockers refuse the migration before anything is written.
...
The same scan is surfaced read-only by [`gofasta refactor status`](#status) (an `Eligibility:` section, plus a `preflight` object in `--json`) and by `gofasta doctor` (project-health entries), so you can check eligibility without attempting a migration. `--dry-run` shows the findings but never refuses — it writes nothing.
