...
- `app/jobs/`, `app/tasks/`, `app/graphql/`, `app/devtools/` — not per-resource.
...
- `app/validators/app_validator.go`, `app/rest/controllers/validator.go`, `app/graphql/resolvers/user.resolvers.go` — `<mod>/app/dtos` import becomes `<mod>/app/shared/dtos` (only the path; the `dtos.` qualifier stays).
...
2. **Resolve resources** — explicit `<Resource>` argument, or `glob app/models/*.model.go` for `--all`.
3. **Per-resource loop** — for each resource, move + featurize-transform every file in the mapping table above; update mocks.
4. **Relocate shared aliases** — `app/dtos/aliases.go` → `app/shared/dtos/aliases.go`.
...
7. **Patch dtos consumers** — app_validator.go, validator.go, resolver.go.
8. **Prune empty layered directories** — `app/services/interfaces`, `app/repositories/interfaces`, `app/di/providers`, etc.
9. **Update config.yaml** — `project.layout: feature`.
10. **Regenerate Wire** — delete `app/di/wire_gen.go`, run `go tool wire ./app/di/`.
11. **Verify** — `go build ./...`. On failure: abort with `REFACTOR_ABORTED`, leave partial state in place.

The migration is **not transactional**: if step 11 fails, files have already been moved and patched. The recovery path is `git restore .` — that's why the dirty-tree check exists.
...
- `app/validators/app_validator.go`, `app/rest/controllers/validator.go`, `app/graphql/resolvers/user.resolvers.go` — `<mod>/app/shared/dtos` → `<mod>/app/dtos`.
...
1. **Always use `gofasta refactor` to switch layouts.** Don't `git mv` files manually — the refactor is doing 11 distinct steps beyond moving (rewrite imports, patch cross-cutting, regenerate Wire, update config.yaml, etc.). Hand-moving leaves the project in a state where the filesystem and `config.yaml` disagree.
