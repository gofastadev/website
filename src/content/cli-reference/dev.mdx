...
`gofasta dev` activates the **`cache` and `queue` compose profiles automatically** so your scaffolded project's redis + asynqmon services come up alongside the database. Use `--services=<list>` to pick an explicit subset (e.g. `--services=postgres` to skip cache + queue), or the [interactive preflight menu](#preflight-menu) to opt out of specific services without typing the list.
...
| `--services=<list>` | *(all non-app services in compose.yaml)* | Comma-separated explicit list of compose services to start. Use this to opt out of specific services (e.g. `--services=cache,queue` to skip the DB when you're running it elsewhere). When set, the [interactive preflight menu](#preflight-menu) is bypassed. |
| `--profile=<name>` | *(none)* | Additional compose profile to activate. `cache` and `queue` profiles are auto-activated when present. |
...
Three ways to control which compose services come up, in priority order:

1. **`--services=a,b,c`** — explicit list. Starts exactly those services and skips the preflight menu. Best for scripts.
2. **Interactive preflight menu** — when stdin is a TTY and `--services` isn't set, gofasta prints the resolved service list with options to (1) launch as-is, (2) deselect specific services, (3) run without DB (degraded mode with a SQLite stub), or (q) quit. See [preflight menu](#preflight-menu) below.
3. **Default** — every non-app service in `compose.yaml` plus the `cache` and `queue` profiles if defined. The `app` service is included only under `--all-in-docker`.

## Preflight menu

When you run `gofasta dev` on an interactive terminal without `--services=<list>` (and without `--no-keyboard`), gofasta resolves the compose service list and prints a menu before launching anything:

```
gofasta dev — preflight

Services to start: postgres, redis, asynqmon

  [1] Launch as-is
  [2] Deselect specific services (then launch)
  [3] Run without DB (degraded mode — uses a SQLite stub)
  [q] Quit

Choice [1]:
```

Pressing `Enter` (or `1`) launches with the default set. `[2]` opens a per-service toggle so you can drop e.g. `asynqmon` without retyping the full list. `[3]` enters **degraded mode** — gofasta starts the cache + queue services (if any) but skips the database, then injects a per-process SQLite stub so the app boots and Air can hot-reload without a database connection. Useful for UI / routing work where you don't need real data.

The menu auto-skips when:

- `--services=<list>` is set — the explicit list wins, no menu shown
- stdin is not a TTY (CI, piped input) — default service list is used
- `--no-keyboard` is set — no menu, no in-loop keystroke handling
...
**Mutual exclusions.** `--all-in-docker` requires the database service in `--services=<list>` (the in-container app needs the database) and requires an `app` service in `compose.yaml` — gofasta scaffolds one by default; if your project's compose file is hand-authored without one, a `DEV_FLAG_CONFLICT` error fires.
...
| `DEV_SERVICE_UNHEALTHY` | A service didn't become healthy within the compose healthcheck deadline |
...
| `DEV_FLAG_CONFLICT` | Two flags asked for incompatible behavior — e.g. `--all-in-docker` against a `compose.yaml` with no `app` service. |
...
# Just the database — drop cache + queue via the explicit services list.
gofasta dev --services postgres
...
# Host-run app with your own Postgres — bypass compose entirely (empty services list).
gofasta dev --services ''

# Debug Air itself — no compose services, no migrations, just the rebuild loop.
gofasta dev --services '' --no-migrate
...
<RelatedPages
  ...
  extra={[
    ...
    { href: "/docs/api-reference/queue", label: "pkg/queue" },
    { href: "/docs/guides/database-and-migrations", label: "Database & Migrations guide" },
