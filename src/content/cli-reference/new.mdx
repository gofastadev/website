...
| `--layout` | `layered` | Project layout: `layered` \| `feature`. `layered` (default) puts each layer in its own directory (`app/models/`, `app/services/`, `app/repositories/`, …). `feature` collapses per-resource files into `app/<resource>/` (Option B hybrid — see [Restructuring to feature modules](/docs/getting-started/project-structure#restructuring-to-feature-modules)). |
