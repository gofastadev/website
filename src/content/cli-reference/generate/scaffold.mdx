...
Generates a full CRUD resource spanning all layers of the application. This is the most powerful generator command -- it creates <Fact name="scaffold.createdCount" /> new files (tests included) and patches <Fact name="scaffold.patchedCount" /> existing files to wire everything together. After running this command and applying migrations, you have a fully working REST API for your resource.
...
| `--swagger` | `false` | Add Swagger/OpenAPI annotations to the generated controller (`@Summary`, `@Tags`, `@Router`, etc. — drives `gofasta swagger`'s spec generation). |
| `--dry-run` | `false` | Show the files that would be created and patched without writing to disk. Honors the global `--json` flag for structured output (a `{create: [...], patch: [...]}` plan). |
| `--no-verify` | `false` | Skip the post-generation `go build ./...` check. The default `go build` catches template-render issues before the user sees them; pass this flag when you're scripting many scaffolds back-to-back and want to defer the build. |
...
Running `gofasta g scaffold Product name:string price:float` creates <Fact name="scaffold.createdCount" /> files:
...
- Generates a GraphQL schema file (`app/graphql/schema/<resource>.gql`)
- Patches `app/graphql/resolvers/resolver.go` to add the service dependency (layout-aware: `svcInterfaces.` in layered projects, the `<resource>pkg.` feature alias in feature projects)
- On feature-layout projects, appends `"<mod>/app/<resource>"` to `gqlgen.yml`'s `autobind` list so the schema types bind against the feature package's DTOs
...
<RelatedPages
  path="cli-reference/generate/scaffold"
  extra={[
