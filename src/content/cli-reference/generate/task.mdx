...
description: Generate an asynq-backed async task — emits a `Task<Name>` constant, a `<Name>Payload` struct, a `Handle<Name>` handler, and an `Enqueue<Name>` helper. Wired into the queue worker so `EnqueueX(ctx, q, p)` and the worker pickup just work.
keywords:
  - "generate task"
  - "async task"
  - "asynq"
...
<RelatedPages
  path="cli-reference/generate/task"
  extra={[
