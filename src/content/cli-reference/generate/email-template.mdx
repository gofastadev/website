...
description: "Generate an HTML email template with a matching Go template file under app/emails/templates/ and a registration entry so the pkg/mailer renderer can find it by name. Wire-friendly: the template renders inside a transaction so you can mail-and-commit cleanly."
keywords:
...
<RelatedPages
  path="cli-reference/generate/email-template"
  extra={[
