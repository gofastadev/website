...
description: Generate a Google Wire dependency-injection provider set for a resource — the small `wire.NewSet(...)` glue that ties the service, repository, and controller together so the Wire-generated container can resolve them.
keywords:
  - "generate provider"
  - "Google Wire"
...
<RelatedPages
  path="cli-reference/generate/provider"
  extra={[
