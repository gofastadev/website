...
Generates a REST controller for a given resource with full CRUD endpoint handlers, along with all prerequisite layers (model, migration, repository, service, DTOs, Wire provider, and routes). The controller delegates all business logic to the service layer and uses gorilla/mux for HTTP routing with `httputil.Handle()` for error-returning handlers.
...
- Route parameters are extracted via `mux.Vars(r)`
