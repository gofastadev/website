...
description: Generate a REST controller (with CRUD handlers) plus every prerequisite layer — model, migration, repository, service, DTOs, Wire provider, and routes. Same shape as `g scaffold` but explicitly focused on the controller side.
keywords:
  - "generate controller"
  - "HTTP handler"
  - "REST controller"
...
<RelatedPages path="cli-reference/generate/controller" />
