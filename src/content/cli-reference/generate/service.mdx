...
description: Generate a service interface and implementation containing business logic, plus every prerequisite layer (model, migration, repository, DTOs, Wire provider). Stops short of the controller / routes — use for background-only services or service-only scaffolds.
keywords:
  - "generate service"
...
<RelatedPages path="cli-reference/generate/service" />
