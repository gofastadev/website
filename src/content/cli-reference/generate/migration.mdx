...
description: Generate a `.up.sql` / `.down.sql` migration pair under `db/migrations/` for a database table — with per-driver SQL flavor (postgres, mysql, sqlite, sqlserver, clickhouse) picked from `config.yaml`. Use this when you want a schema change without a corresponding Go model.
keywords:
  - "generate migration"
  - "database migration"
  - "schema change"
...
<RelatedPages
  path="cli-reference/generate/migration"
  extra={[
