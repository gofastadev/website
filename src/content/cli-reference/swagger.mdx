...
description: Generate OpenAPI/Swagger documentation from the `@Summary`, `@Tags`, `@Router`, and `@Param` annotations the scaffold and `g endpoint` emit. Outputs `docs/swagger.json` + `docs/swagger.yaml` for consumption by Swagger UI, ReDoc, or OpenAPI clients.
keywords:
  - "CLI"
  - "swagger"
  - "OpenAPI"
...
<RelatedPages path="cli-reference/swagger" />
