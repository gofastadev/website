...
It starts a fresh PostgreSQL container, runs the framework's base migrations (the `citext` extension and the shared trigger functions), and returns a ready-to-use `*gorm.DB`. The container is automatically torn down at the end of the test via `t.Cleanup`, so every test gets an isolated database.
...
Controllers are standard `net/http` handlers, so test them with the standard library's `net/http/httptest`. There is no special testing router; use the real `gorilla/mux` router and `httputil.Handle` wrapper.
...
func TestProductController_Create(t *testing.T) {
    ...
    router.HandleFunc("/products", httputil.Handle(ctrl.Create)).Methods(http.MethodPost)
