...
That's it — the framework (`github.com/gofastadev/gofasta/pkg/*`) never imports any router. Middleware in `pkg/middleware` is plain `func(http.Handler) http.Handler`, so it works with any router that accepts `net/http` middleware.
