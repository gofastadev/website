...
Gofasta projects include pre-configured deployment files for multiple targets. The fastest way to deploy is the built-in **[`gofasta deploy`](/docs/cli-reference/deploy)** command, which handles Docker or binary deployments to any VPS via SSH. This guide also covers manual Docker, Kubernetes, and CI/CD setups for more advanced scenarios.

## Quick Deploy with the CLI

The `gofasta deploy` command automates the entire deployment process:

```bash
# First-time server setup (installs Docker, nginx, creates directories)
gofasta deploy setup --host deploy@api.example.com

# Deploy via Docker (default)
gofasta deploy

# Deploy as a compiled binary
...
Configure your deploy target in `config.yaml`:
...
See the [full CLI reference](/docs/cli-reference/deploy) for all options.

## Manual Deployment Options

The sections below cover manual deployment for cases where you need more control or are deploying to managed platforms like Kubernetes or cloud container services.
...
- [gofasta deploy CLI reference](/docs/cli-reference/deploy) -- one-command deploy via SSH
