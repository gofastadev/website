...
For more complex scenarios — user-targeted rollouts, percentage-based releases, A/B tests — the `github.com/gofastadev/gofasta/pkg/featureflag` package wraps [go-feature-flag](https://github.com/thomaspoignant/go-feature-flag) and reads its rules from a YAML file (typically `configs/features.yaml`):

```go
import "github.com/gofastadev/gofasta/pkg/featureflag"

ff, err := featureflag.NewFeatureFlagService("configs/features.yaml", logger)
if err != nil {
    return err
}
defer ff.Close()

if ff.IsEnabled("registration", user.ID.String(), map[string]interface{}{"plan": user.Plan}) {
