...
```
{APP_NAME}_{SECTION}_{KEY}
```

The app name is uppercased from `app.name` in your config. Nested keys are joined with underscores.
...
```bash
MYAPP_JWT_SECRET=$(openssl rand -hex 32)
```
...
MYAPP_JWT_SECRET=staging-secret
```
...
MYAPP_MAIL_SENDGRID_API_KEY=SG.prod-key
```
