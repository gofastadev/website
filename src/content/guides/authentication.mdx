...
// Apply to a route group — chi's Group attaches shared middleware to every
...
api.Group(func(protected chi.Router) {
    ...
    protected.Get("/profile", httputil.Handle(userController.GetProfile))
    protected.Put("/profile", httputil.Handle(userController.UpdateProfile))
})
```
...
api.Route("/api/v1/admin", func(admin chi.Router) {
    ...
    admin.Get("/users", httputil.Handle(adminController.ListUsers))
    admin.Delete("/users/{id}", httputil.Handle(adminController.DeleteUser))
})
```
...
<RelatedPages
  ...
  extra={[
    ...
    { href: "/docs/api-reference/sessions", label: "pkg/session" },
  ]}
/>
