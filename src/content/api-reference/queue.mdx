---
title: pkg/queue — Async task queue powered by asynq + Redis
description: Background job processing built on hibiken/asynq with Redis — task registration, configurable concurrency, retry policies with exponential backoff, scheduled deliveries, queue priorities, and integration with the asynqmon dashboard.
keywords:
  - "queue API"
  - "asynq"
  - "message queue"
...
<RelatedPages path="api-reference/queue" />
