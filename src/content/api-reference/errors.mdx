---
title: pkg/errors — Typed app errors with HTTP status mapping
description: Domain error types (NotFound, BadRequest, Conflict, Internal, Unauthorized, Forbidden) that map automatically to HTTP status codes, with stable error codes for API consumers and a GraphQL error presenter for the GraphQL layer.
keywords:
  - "errors API"
  - "error handling"
  - "HTTP errors"
...
<RelatedPages path="api-reference/errors" />
