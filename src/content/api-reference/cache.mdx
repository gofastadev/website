---
title: pkg/cache — In-memory + Redis caching with TTL
description: Pluggable caching interface backed by an in-memory store (for dev/tests) or Redis (for production). Get / Set / Delete / Flush / Ping API, per-key TTL, automatic eviction, and a shared client suitable for cache-aside, write-through, and rate-limit patterns.
keywords:
  - "cache API"
  - "caching"
  - "Redis cache"
...
<RelatedPages path="api-reference/cache" />
