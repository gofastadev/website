---
title: pkg/seeds — Ordered database seeders for GORM
description: Small registry pattern for database seeding — Register(name, fn) to add a seeder, RunAll() to execute them in registration order. Designed for both fresh-install bootstrap data (default admin user, lookup tables) and test fixtures.
keywords:
  - "seeds API"
  - "database seeding"
  - "GORM seeders"
...
<RelatedPages path="api-reference/seeds" />
