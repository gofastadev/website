---
title: pkg/config — Koanf-backed YAML + env-var configuration
description: Typed configuration loader powered by Koanf — reads config.yaml, applies GOFASTA_-prefixed environment overrides, and exposes a typed AppConfig struct. Includes SetupDB for the five supported drivers (postgres, mysql, sqlite, sqlserver, clickhouse).
keywords:
  - "config API"
  - "configuration"
  - "Koanf"
...
<RelatedPages path="api-reference/config" />
