---
title: pkg/auth — JWT, RBAC, and Casbin authorization
description: JWT generation and validation, refresh tokens, claims management, HTTP auth middleware, and Casbin-backed RBAC role enforcement — the production-ready auth stack baked into every gofasta project.
keywords:
  - "auth API"
  - "authentication"
  - "authorization"
...
carried by the embedded `jwt.RegisteredClaims`. Read it with `SubjectID()`:
...
- It is a **subject**, not necessarily a user. Under the client-credentials
  grant `sub` conventionally holds a client id, so code assuming it names a row
  in your users table is wrong for that grant.
- It is unique **within the issuer**, not globally — the globally unique identity
  is the pair (`iss`, `sub`). If you accept tokens from more than one issuer,
  key on both, or two issuers' subjects can collide into one identity.

Roles come as a single `role` or a `roles` array; `HasRole`, `HasAnyRole` and
`RoleLadder` read both.
...
<RelatedPages path="api-reference/auth" />
