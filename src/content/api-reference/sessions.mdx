...
The session configuration uses koanf struct tags and is part of the root `AppConfig`. Environment variables use the `GOFASTA_` prefix (e.g., `GOFASTA_SESSION_DRIVER`). Sessions use gorilla/sessions under the hood.
...
```bash
export GOFASTA_SESSION_DRIVER=cookie
