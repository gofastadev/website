...
- **`schema/`** — `.gql` schema files. Each resource gets `<r>.gql`.
- **`resolvers/`** — gqlgen-generated resolver stubs. Hand-edited code goes inside the resolver methods. In feature layout, the resolver still lives here (shared, not per-feature); only its imports differ (`userpkg "<mod>/app/user"` vs. layered's `svcInterfaces`).
