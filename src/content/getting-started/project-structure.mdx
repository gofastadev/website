...
You don't have to follow the manual recipe — gofasta ships both a scaffold flag and an automated migration command:

- **`gofasta new my-app --layout=feature`** scaffolds a new project directly in the feature-package layout. Per-resource files (service, repository, controller, routes, dtos, errors, inputs, wire provider) live in `app/<feature>/`; the shared aliases (`TPaginationObjectDto`, `SortOrientation`) live in `app/shared/dtos/`. Models stay in `app/models/` and validator infrastructure stays in `app/validators/` — this is the Option B hybrid documented in [`internal/featurize/featurize.go`](https://github.com/gofastadev/cli/blob/main/internal/featurize/featurize.go), chosen to avoid the `dtos → feature → dtos` import cycle that would otherwise arise from collapsing every layer.

- **`gofasta refactor feature-package <Resource>`** migrates one resource at a time. **`gofasta refactor feature-package --all`** migrates every resource in `app/models/`. After each resource the engine regenerates Wire and runs `go build ./...`; on failure it aborts with the partial state in place so you can `git restore`. The refactor command refuses to run on a dirty working tree unless you pass `--force`, and `--dry-run` previews the plan without writing.

- **`gofasta g scaffold Product …`** detects the project's layout from `config.yaml`'s `project.layout` field and emits the right shape. New resources scaffolded into a feature-layout project land in `app/<resource>/` automatically.

The full reference lives at [`gofasta refactor feature-package`](/docs/cli-reference/refactor).
