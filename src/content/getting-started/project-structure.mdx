...
- **`di/`** — Google Wire dependency injection. `wire.go` defines how to build the container; `wire_gen.go` is the generated output. Tools are registered via `go mod edit -tool` (Go 1.25.8 tool dependencies) and invoked via `go tool wire`, `go tool gqlgen`, etc.
