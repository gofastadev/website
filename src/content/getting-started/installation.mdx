---
title: Installation
description: Install the Gofasta CLI with `go install` or the pre-built binary install script. Covers macOS, Linux, and Windows (WSL); verifying the install with `gofasta doctor`; and the optional shell completion for bash, zsh, and fish.
keywords:
  - "install Gofasta"
  - "CLI install"
  - "pre-built binary"
...
If you don't have Go installed or just want a single self-contained binary, download the archive for your platform from [GitHub Releases](https://github.com/gofastadev/cli/releases) (macOS and Linux, `amd64` and `arm64`), then unpack it onto your `PATH`:
