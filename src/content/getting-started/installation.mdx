---
title: Installation
description: Install the Gofasta CLI with `go install` or a pre-built binary from GitHub Releases. Covers macOS, Linux, and Windows; verifying the install with `gofasta doctor`; and the optional shell completion for bash, zsh, and fish.
keywords:
  - "install Gofasta"
  - "CLI install"
  - "pre-built binary"
...
If you don't have Go installed or just want a single self-contained binary, download the archive for your platform from [GitHub Releases](https://github.com/gofastadev/cli/releases) (macOS, Linux, and Windows, `amd64` and `arm64`). On macOS/Linux, unpack it onto your `PATH`:
...
| You want reproducible pinned versions in CI | Either — both support version pinning (`@v0.1.3` for Option A; download the matching tagged archive from the releases page for Option B) |
...
<RelatedPages path="getting-started/installation" />
