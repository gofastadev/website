...
| Email support | No | No | No | Yes (SMTP, SendGrid, Brevo) |
| Multi-channel notifications | No | No | No | Yes (email, SMS, Slack, database) |
| WebSocket support | No | No | No | Yes |
| File storage | No | No | No | Yes (local, S3) |
| Internationalization | No | No | No | Yes |
...
| Test utilities | No | No | No | Yes (testcontainers) |
...
- **In-process event bus** (`pkg/events`) — Lightweight publish/subscribe within a single service for decoupling side effects (send welcome email after signup, update search index after edit, log analytics event after purchase). Not a message broker abstraction — a simple, typed, in-process dispatcher that covers 80% of event-driven use cases without introducing Kafka or NATS.
...
- **Expand `pkg/testutil`** — HTTP test client, response assertion helpers (`AssertStatus`, `AssertJSON`), fixture loading from JSON/YAML files, and database assertion helpers (`AssertDBHas`, `AssertDBMissing`).
...
- **Message broker abstraction** — Abstracting over Kafka, NATS, and RabbitMQ is Watermill's job. Gofasta's `pkg/events` will provide in-process pub/sub; distributed messaging is left to purpose-built libraries.
