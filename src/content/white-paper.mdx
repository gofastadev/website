...
Go is an excellent language for building backend services, but starting a new production project still involves significant repetitive setup: wiring routers, configuring databases, setting up authentication, writing CRUD boilerplate, organizing project structure, and preparing deployment manifests. Go's explicitness — every behavior visible in source, no hidden control flow — is precisely why engineers choose it; the setup cost is the price of that property, but it does not require each team to pay it independently. Each team reinvents these patterns anyway, producing inconsistent architectures and wasted effort across the ecosystem.
...
3. **A toolkit designed to be driven by AI coding agents as first-class users.** Every scaffolded project ships with an `AGENTS.md` briefing, a JSON-Schema-aware config helper, a structured-error protocol, and an opt-in installer (`gofasta ai <agent>`) that wires per-agent configuration for Claude Code, OpenAI Codex, Cursor, Aider, and Windsurf. The documentation site publishes [`/llms.txt`](https://gofasta.dev/llms.txt) and [`/llms-full.txt`](https://gofasta.dev/llms-full.txt) so agents reason about current features rather than stale training data. This is not a bolt-on layer — it is a design constraint that shapes every CLI command and every scaffold default. See [§2.7 Agent-Native by Default](#27-agent-native-by-default) and [§4.5 Agent Ergonomics](#45-agent-ergonomics).
...
Gofasta projects ship in one of **two layouts**, picked at scaffold time via `gofasta new --layout=<layered|feature>` and persisted to `config.yaml`'s `project.layout` field. The choice is reversible at any time with `gofasta refactor feature` (layered → feature) or `gofasta refactor layered` (feature → layered) — an AST-based migration tool that moves files, rewrites imports, patches Wire bindings, and regenerates `wire_gen.go` in a single transactional pass.

Both layouts produce a fully working REST/GraphQL backend with the same Wire dependency graph, the same HTTP routes, and the same runtime behavior. They differ only in how files are organized on disk:

- **Layered** (the default) groups files by their architectural role: `app/models/`, `app/services/`, `app/repositories/`, `app/rest/controllers/`, etc. Adding a `Subscription` resource touches one file in each of those directories. This is the familiar shape of classic MVC backends — the "controller → service → repository → database" pipeline is one directory per layer, with one file per resource inside each.
- **Feature-package** groups files by business domain: `app/user/`, `app/product/`, `app/order/` — each containing its own service, repository, controller, routes, DTOs, and Wire provider. This is the shape advocated by Ben Johnson's ["Standard Package Layout"](https://www.gobeyond.dev/standard-package-layout/) and the package-by-feature / vertical-slice tradition. Adding a `Subscription` resource creates one directory.

The feature-package layout is a deliberate **hybrid**, not a wholesale collapse. Four categories of files stay in their layered locations even in feature mode:

1. **Models** (`app/models/<r>.model.go`) — DTO mappers (`UserFromModel(m *models.User) *User`) take the model as input; if the model moved into the feature package and the per-resource DTOs also moved there, the controller's `dtos.TPaginationObjectDto` import would create a `dtos → feature → dtos` cycle through the mapper. Keeping models layered breaks the cycle.
2. **Per-resource validators** (`app/validators/<r>.validators.go`) — `app/validators/register.go` calls package-private helpers (`isRecordExistByEmailForConflict`, `doesRecordExistByEmailForVerification`, etc.). Scattering them across feature packages breaks the registration wiring.
3. **Validator infrastructure** (`app/validators/{app_validator,register,custom-messages,validator.utils}.go`) — cross-cutting setup that every feature consumes.
4. **The `Validator` interface** (`app/rest/controllers/validator.go`) — referenced by every feature's controller and bound to `*validators.AppValidator` via `wire.Bind`. Genuinely cross-feature.

Two other things relocate without changing package identity: `app/dtos/aliases.go` (re-exports of `pkg/types`) moves to `app/shared/dtos/aliases.go` (package still `dtos`, just a new import path); `app/services/password_generator.go` moves to `app/user/password_generator.go` (its only consumer in the bootstrap scaffold is the User feature).

Layered shape:

```
# Layered — `gofasta new myapp` (or `--layout=layered`)
myapp/
├── app/
│   ├── main/main.go            # Entry point
│   ├── models/                 # GORM models (one file per resource)
│   ├── dtos/                   # Request/response DTOs + shared aliases.go
│   ├── repositories/
│   │   └── interfaces/         # Repository contracts
│   ├── services/
│   │   ├── interfaces/         # Service contracts
│   │   ├── <r>.service.go      # Business logic
│   │   ├── <r>_errors.go       # Sentinel errors
│   │   ├── <r>_inputs.go       # Domain inputs (CreateUserInput, etc.)
│   │   └── password_generator.go  # Shared infra
│   ├── rest/
│   │   ├── controllers/        # HTTP handlers + Validator interface
│   │   └── routes/             # Per-resource routes + index.routes.go
│   ├── validators/             # AppValidator + per-resource rules
│   ├── di/
│   │   ├── container.go        # ServiceContainer struct
│   │   ├── wire.go             # wire.Build directives
│   │   ├── wire_gen.go         # Generated — do not edit
│   │   └── providers/          # One Wire provider per resource
│   ├── graphql/                # Only with --graphql
│   ├── jobs/                   # Cron jobs (never per-resource)
│   └── tasks/                  # Async asynq handlers (never per-resource)
├── cmd/                        # Cobra CLI (serve, migrate, seed)
├── db/{migrations/, seeds/}
├── configs/                    # RBAC + feature flags
├── deployments/                # Docker, CI, nginx, systemd
├── templates/emails/
├── locales/
├── config.yaml                 # project.layout: layered
├── compose.yaml
├── Dockerfile
├── Makefile
└── gqlgen.yml                  # --graphql only
```

Feature-package shape — same project, files reorganized by domain:

```
# Feature-package — `gofasta new myapp --layout=feature`
myapp/
├── app/
│   ├── main/main.go
│   ├── models/                 # ← stays layered
│   │   └── user.model.go
│   ├── shared/dtos/            # ← shared aliases relocate here
│   │   └── aliases.go
│   ├── user/                   # ← per-feature directory
│   │   ├── dtos.go             # DTOs collapsed in
...
│   │   └── *_test.go
│   ├── validators/             # ← stays layered (shared infra + register.go)
│   ├── rest/
│   │   ├── controllers/validator.go    # ← stays layered (Validator interface)
│   │   └── routes/index.routes.go      # calls userpkg.RegisterRoutes(...)
│   ├── di/
│   │   ├── container.go        # imports app/user; field types use userpkg
│   │   ├── wire.go             # references userpkg.UserSet
│   │   └── providers/core.go   # CoreSet only — no per-resource providers
│   ├── graphql/                # Same path as layered, different imports
│   ├── jobs/, tasks/           # Same as layered
├── cmd/, db/, configs/, deployments/, templates/, locales/   # All identical
├── config.yaml                 # project.layout: feature
├── compose.yaml, Dockerfile, Makefile, gqlgen.yml
```

The full reference — including how `gofasta refactor` performs the bidirectional AST migration, what gets patched in the cross-cutting wiring files, the safety gates that protect the working tree, and the rules to follow to avoid drift — is in [Project structure → Choosing a layout](/docs/getting-started/project-structure#choosing-a-layout-layered-or-feature-package) and [`gofasta refactor`](/docs/cli-reference/refactor).
...
# Option B: Pre-built binary (no Go toolchain needed) — download the
# archive for your platform from the GitHub Releases page and unpack
# the `gofasta` binary onto your PATH.
```

**Requirement:** Go 1.25.0 or later (Option A only). Option B is a self-contained binary from [GitHub Releases](https://github.com/gofastadev/cli/releases).
...
| Command | Description |
|---------|-------------|
| `gofasta new <name>` | Bootstrap a new project (~<Fact name="skeleton.fileCount" /> files) with full structure, Go module, and starter User resource. Accepts a plain name (`myapp`) or a full module path (`github.com/myorg/myapp`). Add `--graphql` / `--gql` to include gqlgen schema and resolvers. |
...
All deploy commands read configuration from the `deploy:` section of `config.yaml` and share one persistent flag set — `--host`, `--method` (`docker` or `binary`), `--port`, `--path`, `--arch`, and `--dry-run` work on the parent command and on every subcommand.

| Command | Description |
|---------|-------------|
| `gofasta deploy` | Deploy the application to a remote server via SSH. Defaults to the `docker` method (transfers a built Docker image and restarts onto it); `--method binary` cross-compiles a Go binary and manages it via systemd. A release that fails its migrations or health check is automatically rolled back to the previous release. |
| `gofasta deploy setup` | Prepare a fresh VPS for deployment: install system packages, Docker (for docker method) or service user + migrate CLI (for binary method), and create the directory structure. Installs the nginx reverse-proxy vhost only when `deploy.domain` is set in `config.yaml`. |
...
| `gofasta deploy rollback` | Restart the service on the newest release older than the current one, verify its health, and only then move the `current` pointer. |
...
2. Runs `go mod init`
3. Copies ~<Fact name="skeleton.fileCount" /> template files (REST-only by default)
...
|-----------|-------|--------|
| `new` | Project name, module path, driver | ~<Fact name="skeleton.fileCount" /> files: full project structure |
| `scaffold` | Resource name, field definitions | <Fact name="scaffold.createdCount" /> new files (tests included) + <Fact name="scaffold.patchedCount" /> patched files |
...
└── systemd/         # Service unit for binary-method deploys
```
...
Because the generated `Dockerfile` and compose file are standard, unmodified Docker artifacts, they also deploy as-is to self-hosted platforms such as Coolify, Dokploy, and Kamal — no gofasta-specific adapters or extra configuration files required. The documentation ships step-by-step guides for each.

### 21.2 Docker

The generated `Dockerfile` uses a multi-stage build. The builder stage caches module downloads (`go.mod`/`go.sum` first), compiles a static binary, and installs the golang-migrate CLI with the project's driver tag; the runtime stage is minimal Alpine running as a non-root user:
...
The development `compose.yaml` includes the application, the configured database, Redis (cache profile), and asynqmon (queue profile); the database and cache services carry container health checks. The development loop is driven by a single command:

```bash
gofasta dev                 # host-first: Air on the host, preflight + interactive recovery
gofasta dev --services db   # database in Docker, app stays on the host
gofasta dev --services all  # everything inside Docker, including Air
```
...
- **Docker** (default): builds a Docker image locally, transfers it via SSH (`docker save | ssh docker load` — no registry required), and restarts a fixed Docker Compose project onto the pinned per-release image. The stable project name keeps container names, the network, and the database volume identical across releases.
- **Binary**: cross-compiles a Go binary, transfers it via SCP, manages it via systemd. Migrations run before cutover with the release's own binary, so a failed migration aborts without touching the running service.

Both methods use a Capistrano-style release-directory layout: `/opt/<app>/releases/<timestamp>/` plus a `current` symlink pointing at the live release. Cutover is restart-based — the service restarts onto the new release (a brief blip, not zero-downtime) and the deploy then gates on the health endpoint (`/health/ready` by default, which verifies real database and cache connectivity). If the new release fails its migrations or health check, gofasta **automatically rolls back** to the previous release and removes the failed release directory; `gofasta deploy rollback` reverts manually using the same mechanics, health-checking the target before committing the pointer to it. Old releases are pruned by name order, always excluding whichever release `current` points at, and each pruned docker release's image is removed with it. The application can optionally be fronted by the generated nginx reverse-proxy config (`gofasta deploy setup` installs it when `deploy.domain` is set).
...
- **Test** — runs on push/PR with a service container matching the project's database driver (SQLite runs container-free); executes the project's full preflight plus `gofasta test --coverage`, uploads coverage to Codecov, and runs `govulncheck`.
- **Release** — on `v*` tag push, cross-compiles binaries for five platforms (linux/darwin/windows), generates checksums, and publishes a GitHub Release.
- **Deploy (VPS)** — on push to `main` or manual dispatch (`workflow_dispatch`), installs the gofasta CLI and runs `gofasta deploy` against the configured VPS — the same command, release layout, health gate, and automatic rollback as a local deploy. Requires `DEPLOY_HOST`, `DEPLOY_USER`, `DEPLOY_SSH_KEY`, and `DEPLOY_PORT` repository secrets.
...
Gofasta is a deliberate trade-off, not a maximalist tool. The trade-off is: accept a curated set of Go libraries — chi, GORM, Wire, slog, OpenTelemetry, and the rest of the `pkg/*` defaults — as the starting point, in exchange for hours-to-minutes setup time and a generated project structure that compiles, tests, and deploys with standard Go tooling. The defaults are swappable in isolation. The toolkit shapes the start of a project, not the end.

**Use Gofasta when:**

- You are starting a new Go backend service — REST, GraphQL, or both — and want to write the first business endpoint within an hour rather than the first week.
- Your project will grow into a modular monolith or a small set of services, and you want the option to split later without committing to a microservice runtime today.
- You want toolkit-level acceleration (scaffolding, code generation, deployment manifests) without surrendering toolchain control: `go build`, `go test`, standard `net/http` middleware, and individually replaceable packages.
- You expect AI coding agents to contribute meaningfully to the project, and you want a toolchain those agents can drive through structured commands rather than scrape from prose.

**Use something else when:**

- You are building a pure-microservices system with first-class gRPC, Protocol Buffers, and service discovery — [Kratos](https://github.com/go-kratos/kratos) or [go-zero](https://github.com/zeromicro/go-zero) are purpose-built for that workload.
- Your organization is API-design-first and needs OpenAPI plus gRPC generated from a single source of truth — use [Goa](https://goa.design).
- You want a fully managed cloud platform where infrastructure is provisioned from your code at deploy time — use [Encore](https://encore.dev), accepting its proprietary compiler and runtime.
- You are writing a one-off script, CLI utility, or library that does not need a backend service at all — use the standard library directly.

For the broad case in between — a production-ready Go backend that needs to ship quickly, remain readable to anyone familiar with standard Go, and stay portable across hosting environments — Gofasta compresses the setup step without becoming the system. After scaffolding, the project is plain Go. The developer owns it, modifies it, and ships it on their own terms.
