...
| Capability | mux/chi/Gin/Echo | go-blueprint | Buffalo | Encore | Gofasta |
|-----------|-------------|-------------|---------|--------|---------|
| HTTP routing | Yes | Yes | Yes | Yes (annotations) | Yes |
| Project scaffolding | No | Yes | Yes | Yes | Yes |
| Resource code generation | No | No | Yes | No | Yes |
| Compile-time DI | No | No | No | No | Yes (Wire) |
| Database migrations | No | No | Yes (Pop) | Yes (auto-provisioned) | Yes (SQL files) |
| Multi-database support | N/A | Yes | Yes (Pop) | No (PostgreSQL only) | Yes (5 drivers) |
| GraphQL support | No | No | No | No | Yes (opt-in, gqlgen) |
| Background jobs | No | No | Yes | Yes (cron) | Yes |
| Pub/Sub | No | No | No | Yes (cloud-agnostic) | No (roadmap) |
| Email support | No | No | No | No | Yes (SMTP, SendGrid, Brevo) |
| Multi-channel notifications | No | No | No | No | Yes (email, SMS, Slack, database) |
| WebSocket support | No | No | No | No | Yes |
| File storage | No | No | No | Yes (S3/GCS) | Yes (local, S3) |
| Internationalization | No | No | No | No | Yes |
| Feature flags | No | No | No | No | Yes |
| Resilience patterns | No | No | No | No | Yes |
| Observability | No | No | No | Yes (auto-instrumented) | Yes (Prometheus, OpenTelemetry) |
| Security middleware | Partial | No | Partial | Partial (auth handler) | Yes (headers, CORS, rate limit, auth) |
| Health checks | No | No | No | No | Yes |
| Test utilities | No | No | No | Yes (auto test DB) | Yes (testcontainers) |
| Deployment manifests | No | No | No | No (cloud-managed) | Yes (Docker, K8s, CI/CD, VPS) |
| VPS deployment command | No | No | No | No | Yes |
| Auto infrastructure provisioning | No | No | No | Yes | No |
| Local dev dashboard | No | No | No | Yes | No (roadmap) |
| Standard `go build` / `go test` | Yes | Yes | Yes | No (custom compiler) | Yes |
| `net/http` middleware compatible | Yes | Yes | Yes | No | Yes |
| Runtime wrapper required | Yes | No | Yes | Yes (custom runtime) | No |
| Custom syntax or annotations | No | No | No | Yes (`//encore:api`) | No |
| Free and open source | Yes | Yes | Yes | Partial (cloud is paid) | Yes |
