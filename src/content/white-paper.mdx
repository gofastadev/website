...
- **Layered** (the default) groups files by their architectural role: `app/models/`, `app/services/`, `app/repositories/`, `app/rest/controllers/`, etc. Adding a `Subscription` resource touches one file in each of those directories. This is the familiar shape from MVC backends in Rails, Laravel, NestJS, Django, etc. — the "controller → service → repository → database" pipeline is one directory per layer, with one file per resource inside each.
