...
- **Every scaffolded project ships with `cmd/schema/main.go`** — a 10-line helper that emits the JSON Schema for `config.yaml`, invoked by `gofasta config schema`. Agents validate proposed config edits before writing them, instead of guessing key names from stale training data.
...
- **`cmd/schema/main.go`** — a 10-line helper that prints the JSON Schema for `config.yaml`. Invoked by `gofasta config schema`; callable directly as `go run ./cmd/schema` for CI or IDE extensions.
